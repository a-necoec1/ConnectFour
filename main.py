...
def checkForWinner(chip):
  ...
  for y in range(cols-3):
    for x in range(rows):
      if (gameBoard[x][y] == chip and gameBoard[x][y+1] == chip and gameBoard[x][y+2] == chip and gameBoard[x][y+3] == chip):
        print("\nGame Over!", chip, " wins! Thanks for playing! :D")
        return True
  ...
  # Check diagnol spaces (upper right to bottom left)
  for y in range(cols-3):
    for x in range(rows-3):
  ...
  # Check diagnol spaces (upper left to bottom right)
...
def gravityCheck(intendedCoordinate):
  # Calculate space below
  spaceBelow = [None] * 2
  spaceBelow[0] = intendedCoordinate[0] + 1
  spaceBelow[1] = intendedCoordinate[1]
  # is the space below available?
  if(spaceBelow[0] == 6):
    return True
  # is there a chip below?
...
while True:
  if(turnCounter % 2 == 0):
    ...
    while True:
      ...
      try:
        ...
        if(isSpaceAvailable(coordinate) and gravityCheck(coordinate)):
  ...
  # It's the computer's turn!
    ...
    while True:
      ...
      if(isSpaceAvailable(cpuCoordinate) and gravityCheck(cpuCoordinate)):
